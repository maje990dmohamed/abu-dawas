...

      {
        ...
        />
          ...
          <div className={`w-full gap-2  flex flex-col rounded-[16px] relative`}>
            <label
              className={`lg:text-[16px] font-[500] my-1 text-[#071200] dark:text-primary ${isRTL ? "text-right me-1" : "text-left ms-1"
                }`}
              dir={isRTL ? "rtl" : "ltr"}
            >
              {label} :
              {isRequired && (
                <span className={`text-[#E33629] ${isRTL ? "me-2" : "ms-2"}`}>*</span>
              )}
            </label>

            <div
              className={`flex items-center overflow-hidden !rounded-[16px] w-full ${isRTL ? "flex-row" : "flex-row"
                }`}
              dir={isRTL ? "rtl" : "ltr"}
            >
              <div
          ...
          `}
              ...
              <div className="relative overflow-hidden w-full bg-[#F1F1F1] dark:!bg-table-body ">
                <Select
                  multiple
                  name={name}
                  disabled={disabled}
                  ...
                  className="w-full !h-[48px] overflow-hidden !rounded-e-[16px] !bg-[#F1F1F1] dark:!bg-table-head"
                  sx={{
                    height: "49px",
                    ...
                    "& .MuiOutlinedInput-notchedOutline": {
                      ...
                      borderLeft: isRTL ? `1px solid ${error ? "red" :  "#ccc"}` : "0",
                      borderRight: isRTL ? "0" : `1px solid ${error ? "red" :  "#ccc"}`,
                    },
                ...
                >
                  {availableOptions?.length > 0 ? (
                    availableOptions?.map((option) => (
                      ...
                      >
                        <Checkbox checked={false} className=" dark:!text-white  " />
                        <ListItemText sx={{ textAlign: isRTL ? "right" : "left" }} primary={option.name} />
                      </MenuItem>
                    ))
                  ) : (
                    <MenuItem
                    ...
                    >
                      <ListItemText className=" text-black dark:text-white" primary={"لا يوجد خيارات"} />
                    </MenuItem>
                  )}
                </Select>
                { }
            ...
            <div className="flex absolute top-[105%] overflow-x-auto max-w-full gap-1">
